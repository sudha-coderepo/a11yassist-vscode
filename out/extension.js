...
function activate(context) {
    ...
    const showKeyboardShortcutsCommand = vscode.commands.registerCommand('a11yassist.showKeyboardShortcuts', async () => {
        await analyticsManager.trackEvent('keyboard_shortcuts_viewed');
        keyboardNavigationManager.showKeyboardGuide();
    });
    // Toggle Screen Reader Mode
    const toggleScreenReaderCommand = vscode.commands.registerCommand('a11yassist.toggleScreenReaderMode', async () => {
    ...
    const showAccessibilityPanelCommand = vscode.commands.registerCommand('a11yassist.showAccessibilityPanel', async () => {
    ...
    const announceCurrentContextCommand = vscode.commands.registerCommand('a11yassist.announceCurrentContext', async () => {
        await analyticsManager.trackEvent('context_announced');
        screenReaderManager.announceCurrentContext();
    });
    // Describe Current Element
    const describeElementCommand = vscode.commands.registerCommand('a11yassist.describeElement', async () => {
    ...
    const clearStatisticsCommand = vscode.commands.registerCommand('a11yassist.clearStatistics', async () => {
        const confirm = await vscode.window.showWarningMessage('Clear all accessibility audit statistics and analytics data?', { modal: true }, 'Clear');
        if (confirm === 'Clear') {
    ...
    context.subscriptions.push(runAuditCommand, showKeyboardShortcutsCommand, toggleScreenReaderCommand, provideFeedbackCommand, showAccessibilityPanelCommand, announceCurrentContextCommand, describeElementCommand, navigateToIssueCommand, showAnalyticsSummaryCommand, clearStatisticsCommand);
